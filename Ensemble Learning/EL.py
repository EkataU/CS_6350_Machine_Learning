# -*- coding: utf-8 -*-
"""
Created on Fri Oct 21 07:48:44 2022
...
def _run_ada_boost():
    ...
    y_train = np.array(Make_DT_ID3.train_data[-1])
    y_test = np.array(Make_DT_ID3.test_data[-1])
    train_err_str = ""
    test_err_str = ""
    ...
    test_err_str = ""
    for t in range(T):
        train_err_str += str(Make_DT_ID3.predict(Make_DT_ID3.train_data, trees[t].root)) + ","
        test_err_str += str(Make_DT_ID3.predict(Make_DT_ID3.test_data, trees[t].root)) + ","
...
def _train_iter_ada_boost(t, y):
    ...
    global vote_arr
    s = Make_DT_ID3.train_data
    tree = Make_DT_ID3.train(s, t)
    trees.append(tree)
...
def _get_error(y, t):
    ...
    return 0.5 - (0.5 * (np.sum(Make_DT_ID3.example_wt[t] * y * predictions[t])))
...
def _get_weights(votes, y, t):
    ...
    Make_DT_ID3.example_wt[t+1] = Make_DT_ID3.example_wt[t] * np.exp(-votes[t] * y * predictions[t])
    z = np.sum(Make_DT_ID3.example_wt[t+1])
    Make_DT_ID3.example_wt[t+1] /= z
...
def _run_bagged():
    ...
    for t in range(T):
        [train_err, predictions] = _run_bagged_iter(t, True, Make_DT_ID3.train_data, predictions)
        train_err_str += str(train_err) + ","

        [test_err, test_predictions] = _run_bagged_iter(t, False, Make_DT_ID3.test_data, test_predictions)
...
def _run_bagged_iter(_t, is_train, data, _predictions):
    ...
    if is_train: trees.append(Make_DT_ID3.train(s, _t, attr_subset_num))
...
def _bias_variance_decomp():
    ...
    for i in range(100):
        ...
        for t in range(T):
            s = _random_sample(Make_DT_ID3.train_data)
            trees[i].append(Make_DT_ID3.train(s, t))
            if t == 0: _trees.append(trees[i][t])

    test_predictions = np.zeros((100, m))
    y = np.array(Make_DT_ID3.test_data[-1], dtype=int)
    for i in range(len(_trees)):
        test_predictions[i] = _get_predictions(Make_DT_ID3.test_data, _trees[i].root, test_predictions[i])
...
def _get_predictions(s, root, _predictions):
    ...
    for index in range(len(s[-1])):
        ...
        p[index] = Make_DT_ID3.predict_example(example, root, False)
    return p
...
if __name__ == '__main__':
    Make_DT_ID3.DL_select = "bank"
    alg_type = sys.argv[1]

    if alg_type == "ada":
        Make_DT_ID3.max_depth = 2
        predictions = np.zeros((T, m))
    else:
        predictions = np.zeros(4999)
        m = 2500

    Make_DT_ID3.setup_data(m, T)
